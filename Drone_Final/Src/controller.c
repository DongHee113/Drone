...
void init_pid_gain()
{
	pid_gain.stab_kp[ROLL] = 6;
	pid_gain.rate_kp[ROLL] = 6;
	pid_gain.rate_ki[ROLL] = 0;
	pid_gain.rate_kd[ROLL] = 0;

	pid_gain.stab_kp[PITCH] = 6;
	pid_gain.rate_kp[PITCH] = 6;
	pid_gain.rate_ki[PITCH] = 0;
	pid_gain.rate_kd[PITCH] = 0;

	pid_gain.stab_kp[YAW] = 6;
	pid_gain.rate_kp[YAW] = 6;
...
void alti_pid_controller()
{
	...
	error = desired_alt_vel - (kf_alt.x_v*100.0f);
