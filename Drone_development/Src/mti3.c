...
void decode_mti3()
{
	if(checksum() == 1)
	{
		...
		mti3.acc[ACC_X] = mti3_union.data;
		...
		mti3.acc[ACC_Y] = -mti3_union.data;
		...
		mti3.acc[ACC_Z] = -mti3_union.data;

		/* acceleration */
		mti3_union.byte[0] = mti3_packet[40];
		mti3_union.byte[1] = mti3_packet[39];
		mti3_union.byte[2] = mti3_packet[38];
		mti3_union.byte[3] = mti3_packet[37];

		/* multiple "180/PI" for converting Radian to Degree(mti에서는 라디안 단위로 자이로 값 제공하므로 도 단위로 변환) */
		mti3.pqr[PQR_X] = mti3_union.data * 180/PI;

		mti3_union.byte[0] = mti3_packet[44];
		mti3_union.byte[1] = mti3_packet[43];
		mti3_union.byte[2] = mti3_packet[42];
		mti3_union.byte[3] = mti3_packet[41];

		mti3.pqr[PQR_Y] = -mti3_union.data * 180/PI;

		mti3_union.byte[0] = mti3_packet[48];
		mti3_union.byte[1] = mti3_packet[47];
		mti3_union.byte[2] = mti3_packet[46];
		mti3_union.byte[3] = mti3_packet[45];
